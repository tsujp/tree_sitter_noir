...
fix attribute content parsing

- Attributes may have no contents so add optional to rule.
- Attribute contents structured in such a way that attempts to parse a
  line_comment within them was occuring, this is invalid and broke the parser
  where attributes contained URLs (the // in a URL looks like a line_comment).
- A 'proper' long-term solution might be to use an external scanner for string
  contents, and/or attribute contents but for now this resolves the issue
  correctly and the CST still appears nice so K.I.S.S.
- Also the locus error reports line 136 but is actually from a failure to
  parse the attribute on (real) line 30 of tecurve.nr.

- Noirc stdlib parse progress: 89.86% -> 89.86% (+0%)
- Resolves error attempting to parse Noirc stdlib at locus:
