...
refactor QuoteExpression to external scanner

- It's probably possible to do purely in the tree-sitter DSL but... beats me. I
  know how to do it via external scanner and this commit does so. Unquote is not
  added yet because that adds a lot of (relative) complexity. Will get to that
  later.

- Noirc stdlib parse progress: 94.20% -> 98.55% (+4.35%)
- Resolves error attempting to parse Noirc stdlib at loci:

    noir/noir_stdlib/src/meta/expr.nr       569,46 - 569,51
    noir/noir_stdlib/src/meta/expr.nr       544,23 - 545,1
