...
allow PathNoTurbofish in Constructor name; add tests

- Unclear on exactly which Types are allowed but it appears at least a Path
  without Turbofish is given the error locus here. I believe I already have some
  internal documentation for a Type which is parsed as an Identifier but I
  cannot find it again right now, so, for now the rule
  identifier_or_path_no_turbofish is fine.

- Noirc stdlib parse progress: 89.86% -> 94.20% (+4.34%)
- Resolves error attempting to parse Noirc stdlib at locus:

    noir/noir_stdlib/src/ec/tecurve.nr      99,22 - 99,29
