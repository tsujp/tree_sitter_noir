...
*************** END
*************** TODO Test scenarios
If =:= is present and there's no following trait bound: that's an error
*************** END

****** TOIMPL TraitBody                                                :node:
:PROPERTIES:
...
#+name: trait_body
#+begin_src js :arg $ :ast TraitBody
seq(
    '{',
...
#+name: trait_item
#+begin_src js :_rule __trait_item :arg $ :ast TraitItem
choice(
    <<r("trait_type")>>,
    <<r("trait_constant")>>,
    <<r("trait_function")>>,
...
Even though the literal Noirc parser has already checked =:= before it's called we place that token in our tree-sitter rule to simplify the grammar a little. There are only two possible combinations of this rule appearing (according to Noirc comments).
...
:END:
:pgd:
+ =if= (kw) by [[#h:167542CC-8CDE-49C1-9465-E5157C6A176F][Expression]] by [[#h:B34CADE5-71B4-41F2-AB54-00483EE593A6][BlockExpression]]
+ (opt grp) =else= (kw) by
  - [[#h:B34CADE5-71B4-41F2-AB54-00483EE593A6][BlockExpression]]
...
seq(
    ...
    field('consequence', <<r("block_expression")>>),
    optional(seq(
        'else',
        field('alternative', choice(<<r("block_expression")>>, <<self()>>)),
    )),
)
#+end_src

Parser: [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::if let Some(kind) = self.parse_if_expr() {][parse_if_expr() parent callsite]], [[file:noir/compiler/noirc_frontend/src/parser/parser/expression.rs::pub(super) fn parse_if_expr(&mut self)][parse_if_expr()]]
...
**** BLOCK StatementKind                                       :node:cluster:
:PROPERTIES:
...
Parser: [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::if self.eat_keyword(Keyword::Continue) {][parse continue statement parent inline]]

***** DONE ReturnStatement                                             :node:
...
:END:
:pgd:
+ =return= (kw) by (opt) [[#h:167542CC-8CDE-49C1-9465-E5157C6A176F][Expression]]
:end:
...
Parser: [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::if self.eat_keyword(Keyword::Return) {][parse return statement parent inline]]
...
*************** TODO I thought return statement is always an error in noir since no early returns
*************** END
*************** TODO Does the BNF doc have an error is is code missing that makes it actually optional?
*************** END

***** SPEC LetStatement                                                :node:
:PROPERTIES:
...
Parser: [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::if self.at_keyword(Keyword::Let) {][parse let statement parent callsite]], [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::fn parse_let_statement(&mut self, attributes: Vec<(Attribute, Span)>)][parse_let_statement()]]
...
*************** END
*************** TODO Noirc doc for pattern needs to be capital P since it's a proper AST node?
*************** END

***** DONE ConstrainStatement                                          :node:
...
:END:
:pgd:
+ =assert= (kw) OR =assert_eq= (kw)
+ [[#h:FB4BC935-116B-4C56-B866-3842C5CA9B5F][Arguments]]
:end:
...
:END:
:pgd:
+ [[#h:5092AC79-344E-465F-973E-70DC2ECF38FB][ComptimeBlock]] OR [[#h:46D5DE09-D9EB-4227-BB31-BA77B8B0063D][ComptimeLet]] OR [[#h:B0F78DC8-11C8-4D7E-A72C-7C3498574225][ComptimeFor]]
:end:

#+name: comptime_statement
#+begin_src js :arg $ :ast ComptimeStatement
seq(
    'comptime',
    choice(
        <<r("block_expression")>>, // Noirc: ComptimeBlock
        <<r("let_statement")>>, // Noirc: ComptimeLet
        <<r("for_statement")>>, // Noirc: ComptimeFor
    ),
)
#+end_src
...
:END:
:pgd:
+ =comptime= (kw) by [[#h:B34CADE5-71B4-41F2-AB54-00483EE593A6][BlockExpression]]
:end:
...
:END:
:pgd:
+ =comptime= (kw) by [[#h:FD70AFA1-5D45-434E-A9D0-7E8F6DD38058][LetStatement]]
:end:
...
:END:
:pgd:
+ =comptime= (kw) by [[#h:F444339D-00F1-4553-ACF6-DE0B5E2EE35E][ForStatement]]
:end:

**Inlined unless something else references it in a way that requires otherwise**.

***** SPEC ForStatement                                                :node:
:PROPERTIES:
...
:END:
:pgd:
+ =for= (kw) by [[#h:65B0176B-B46F-4679-8535-C504870AC048][ident]] (token) by =in= (kw) by [[#h:C859B697-6DDD-4F6D-B70B-BF0A9FBA6E80][ForRange]] by [[#h:B34CADE5-71B4-41F2-AB54-00483EE593A6][BlockExpression]]
:end:
...
seq(
    ...
    field('body', <<r("block_expression")>>),
)
#+end_src

Parser: [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::if let Some(for_loop) = self.parse_for() {][parse for parent callsite]], [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::fn parse_for(&mut self)][parse_for()]]
...
+ (grp):
  - === (token)
  - any of the token combinations valid in ~next_is_op_assign~
+ [[#h:167542CC-8CDE-49C1-9465-E5157C6A176F][Expression]]
:end:
...
***** BLOCK ExpressionStatement                                        :node:
:PROPERTIES:
...
Expression statements will appear alongside statements in the tree-sitter grammar instead of nested within them as is depicted in the parser frontend. Implemented at [[#h:788ECB8C-0A48-4255-B35E-F5850902AA32][Statement]].
...
:END:
:pgd:
+ =(= by (opt) [[#h:10AA2FCE-2B45-442A-8349-1161B113A661][ArgumentsList]] by =)=
:end:
...
seq(
    '(',
    sepBy(<<r("expression")>>, ','), // Inlined Noirc: ArgumentsList.
    optional(','),
    ')',
)
#+end_src

Parser: [[file:noir/compiler/noirc_frontend/src/parser/parser/arguments.rs::pub(crate) fn parse_arguments(&mut self)][parse_arguments()]]
...
:END:
:pgd:
+ =!= by [[#h:FB4BC935-116B-4C56-B866-3842C5CA9B5F][Arguments]]
:end:

#+name: call_arguments
#+begin_src js :arg $ :ast CallArguments
seq(
    optional('!'),
    <<r("arguments")>>,
)
...
Parser: [[file:noir/compiler/noirc_frontend/src/parser/parser/arguments.rs::pub(super) fn parse_call_arguments(&mut self)][parse_call_arguments()]]
...
*** [[file:noir/compiler/noirc_frontend/src/parser/parser/statement.rs::fn next_is_op_assign(&mut self)][Parser.next_is_op_assign()]]
:PROPERTIES:
...
*** [[file:noir/compiler/noirc_frontend/src/parser/parser.rs::fn comptime_mutable_and_unconstrained_not_applicable(&mut self, modifiers: Modifiers)][Parser.comptime_mutable_and_unconstrained_not_applicable(modifiers)]]
:PROPERTIES:
...
<<c('("impl" "trait_impl"))>>
<<c('("trait" "trait_body" "trait_item" "trait_type" "trait_constant" "trait_function"))>>
<<n("global")>>
// TODO: TypeAlias
...
#+begin_src js :noweb yes
<<n("statement")>>
<<c('("statement_kind" "break_statement" "continue_statement" "return_statement" "let_statement" "constrain_statement" "comptime_statement" "for_statement" "assign_statement" "expression_statement"))>>
#+end_src
