...
seq(
    optional(<<r("item_visibility")>>),
    repeat(choice(<<r("modifier_comptime")>>, <<r("modifier_unconstrained")>>)),
    'fn',
    field('name', <<r("identifier")>>),
    field('type_parameters', optional(<<r("generic_parameters")>>)),
    field('parameters', <<r("function_parameters")>>),
    optional(seq('->', optional(<<r("visibility")>>), <<r("type")>>)),
    optional(<<r("where")>>),
...
Parent callsites:
  - [[file:noir/compiler/noirc_frontend/src/parser/parser/expression.rs::if let Some(kind) = self.parse_block() {][parse_block() parent parse_literal()]] (for: [[#h:3446FD3E-4526-4DBF-80AB-64BD9078C4F5][Literals]])
  - [[file:noir/compiler/noirc_frontend/src/parser/parser/function.rs::Some(self.parse_block().unwrap_or_else(empty_body))][parse_block() parent parse_function_definition_with_optional_body()]] (for: [[#h:B3C4609F-307A-42A1-B420-DBBAB6CDE6E5][Function]])
Aliases: BlockExpression, BlockStatement
...
#+begin_src js :rule unsafe_block :arg $ :ast UnsafeExpression
seq('unsafe', <<r("block")>>)
#+end_src
...
<<n("global")>>
// TODO: TypeAlias
<<c('("function" "function_parameters" "function_parameter"))>>
...
#+begin_src js :noweb yes
<<n("statement")>>
<<c('("break_statement" "continue_statement" "return_statement" "let_statement" "constrain_statement" "comptime_statement" "for_statement" "assign_statement" "expression_statement"))>>
#+end_src
